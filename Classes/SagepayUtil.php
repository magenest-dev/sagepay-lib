...
//        }
     ...
     */
    static public function encryptAesOld($string, $key)
    {
    ...
    static public function encryptAes($string, $key)
    {
        ...
        $hexaText   = bin2hex($binCipherText);
        $uppercaseHexaText = strtoupper($hexaText);

        return "@$uppercaseHexaText";
    }
     ...
     * @throws SagepayApiException
     */
    static public function decryptAesOld($strIn, $password)
    {
        ...
        //$string2 = mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $password, $strIn, MCRYPT_MODE_CBC, $strInitVector);
    ...
    static public function arrayToQueryStringRemovingSensitiveData(array $data,array $nonSensitiveDataKey, $delimiter = '&', $urlencoded = false)
    {
        ...
        foreach ($data as $name => $value)
        {
           if (!in_array($name, $nonSensitiveDataKey)){
				$value= self::MASK_FOR_HIDDEN_FIELDS;
		   }
		   else if ($urlencoded){
				$value = urlencode($value);
		   }
   ...
   static public function queryStringToArrayRemovingSensitiveData($data, $delimeter = "&", $nonSensitiveDataKey)
    {  
        ...
        foreach ($pairs as $pair)
        {
            ...
            if (in_array($key, $nonSensitiveDataKey)){
			  ...
			  $keyValue = array(self::MASK_FOR_HIDDEN_FIELDS);
			}
		    // Implode others as value for $key
			$queryArray[$key] = implode('=', $keyValue);
    ...
    static public function log($message)
    {
        ...
        {
            $filename = Constants::SAGEPAY_SDK_PATH . '/debug.log';
